...
public class BoundingBox {
	...
	// NOTE: do not confuse BoundingBox(0) with BoundingBox.empty!
	...
	public BoundingBox extend(BoundingBox bb) {
		return new BoundingBox(
			point(Math.min(min.x(), bb.min.x()), Math.min(min.y(), bb.min.y()), Math.min(min.z(), bb.min.z())),
	...
	public BoundingBox extend(Point p) {
		return new BoundingBox(
			point(Math.min(min.x(), p.x()), Math.min(min.y(), p.y()), Math.min(min.z(), p.z())),
	...
	public boolean contains(BoundingBox bb) {
		return min.x() <= bb.min.x() && min.y() <= bb.min.y() && min.z() <= bb.min.z() &&
	...
	public boolean intersect(Ray ray) {
		...
		double tz1 = (min.z() - ray.getDirection().z()) * diz;
		double tz2 = (max.z() - ray.getDirection().z()) * diz;
